...
long default_sec;
string filePath = "delay_gt.txt";
ofstream writeFile(filePath.data());
#endif

#ifdef FINDMTX
string filePath = "client_gt.txt";
ofstream writeFile(filePath.data());
#endif

#ifdef STEPCHECK
string filePath2 = "stepcheck_gt.txt";
...
//                          cout<<"mu[3]:"<<m_uRecvPacket.stData.mu[3]<<endl;
                    }
                    m_check = 0;
                    m_mode = 0;
                    m_checkSize = 0;
...
void tb0_trans(nav_msgs::Odometry odom) {
    ...
    */
    if(g_mode != pre_g_mode&&(odom.pose.pose.position.x*odom.pose.pose.position.x+odom.pose.pose.position.y*odom.pose.pose.position.y+odom.pose.pose.position.z*odom.pose.pose.position.z>0.001)){ // Memorize Current Odometry of AR Marker
        ...
        g_mtx = TF.getBasis();
        /*
         * Write [Orientation + Position]MTX -> When Odom & Button
        */
        if(writeFile.is_open()&&(g_mode == pre_g_mode)){
            writeFile<<(unsigned int)pre_g_mode<<" ";
            for (int i = 0; i<3; i++) {
                g_vec = g_mtx[i]; // 3X3 MTX - ROW value RETURN
                writeFile<<g_vec.getX()<<" "<<g_vec.getY()<<" "<<g_vec.getZ()<<" ";
            }
            writeFile<<g_pos[0]<<" "<<g_pos[1]<<" "<<g_pos[2]<<endl;
            if(g_mode > 200){
                writeFile.close();
            }
        }
    ...
    /*
     * g_rbpos : Nothing
     * g_theta : Compare with theta from state estimation
     * g_y : Algorithm's Measurement Value(xi)
     * g_uRecv_odom.stData.y_i : SendPacket to Server
     * g_yi : Nothing
    */
...
{
    ros::init(argc, argv, "estim_client_gt");
    ros::NodeHandle nh;
    ros::Rate rate(HZ); // 5ms
    ros::Subscriber sub = nh.subscribe("/gt_3/odom", 1, tb0_trans);
...
//        cout<<"sv_gmode: "<<g_svmode<<" gt_gmode: "<<g_mode <<"l="<<l<<endl;
