...
float g_gainK = 1.0;
float g_gaintest = 1.0;
static Eigen::MatrixXf d(2,1);
static Eigen::MatrixXf r(2,1);
static Eigen::MatrixXf R(2,2);
...
{
    ros::init(argc, argv, "async_client1");
    ros::NodeHandle nh;
    ros::Rate rate(HZ); // 10ms

    ros::Subscriber sub = nh.subscribe("/tb3_1/odom", 1, tb1_trans);
    ros::Publisher pub = nh.advertise<geometry_msgs::Twist>("/tb1/cmd_vel",10);
...
//        cout<<"//RECV-while//"<<endl;
//        cout<<"leader: "<<g_leader_theta<<"\tK: "<<g_gainK<<"\tmode: "<<(int)pk.stData.mode<<endl;
#endif
