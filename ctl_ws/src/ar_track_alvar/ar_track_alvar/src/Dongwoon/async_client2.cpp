...
{
    ros::init(argc, argv, "async_client2");
    ros::NodeHandle nh;
    ros::Rate rate(HZ); //HZ

    ros::Subscriber sub = nh.subscribe("/tb3_2/odom", 1, tb2_trans);
    ros::Publisher pub = nh.advertise<geometry_msgs::Twist>("/tb2/cmd_vel",10);
...
//        cout<<"leader:"<<g_leader_theta<<"\tK:"<<g_gainK<<"mode:"<<pk.stData.mode<<endl;
        ...
        lin_vel += Out(0)*0.01;
//        cout<<"gth\t"<<g_theta<<"zi\t"<<Z_i(0)<<"xi0\t"<<xi_i(0)<<"out"<<Out(0)<<","<<Out(1)<<endl;
